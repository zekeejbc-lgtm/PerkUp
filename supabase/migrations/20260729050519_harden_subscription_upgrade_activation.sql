alter table public.system_runtime_config
  add column if not exists subscription_upgrades_enabled boolean not null default false,
  add column if not exists subscription_upgrades_changed_at timestamptz,
  add column if not exists subscription_upgrades_changed_by uuid
...
create index if not exists system_runtime_config_subscription_upgrades_changed_by_idx
  on public.system_runtime_config (subscription_upgrades_changed_by)
  where subscription_upgrades_changed_by is not null;
...
begin
  ...
  insert into public.audit_events (
    ...
    case
      when exists (select 1 from auth.users account where account.id = p_actor_user_id)
...
drop function if exists public.confirm_subscription_upgrade(
...
begin
  ...
  for share;
  if not found or catalog_updated_at is distinct from p_plan_catalog_updated_at then
